...
//    printf("initialBisection function called with inipart = %d\n",opt.inipart);
    ...
    for (int run = 0; run < nruns; ++run) {
        ...
        switch (iniAlg) {
            ...
            case IP_RECBISS:
            {
                ...
                rMLGP_info infoini;
                infoini.depth=0;
                idxType resultingNbPartIniBis = -1;
                initRInfoPart(&infoini);

                rcoarsen * rcoars = rVCycle(coars->graph, *optini, &infoini,&resultingNbPartIniBis);
        ...
        if (opt.print>=PD_HIGH)
            printf("InitPart:%d:%s Cut=%lld\n", run, iniAlgName[iniAlg], (long long)edgecut);
    ...
    if (opt.print>=PD_MEDIUM)
        printf("InitPart:Final:%s Cut=%lld\n", iniAlgName[opt.inipart], (long long)edgecutmin);
    if (opt.debug){
