...
void printInfoPart(MLGP_info* info, int nVrtx, int nEdge, int printdetail)
{
    ...
    printf("Final edgecut : %lld \n", (long long) info->final_ec);
    printf("Total time    : %.2f seconds\n", info->timing_global);

    if (significantTime(info->timing_conpar, 0)){
      printf("\nConPar\t :\t %.2f seconds (Edgecut %lld)\n", info->timing_conpar, (long long) info->conpar_ec);
    }
    ...
    if (printdetail > PD_HIGH) {
        ...
        printf("Best Initial Partition : %s with edgecut %lld\n", iniAlgName[info->best_inipart], (long long) info->ini_ec);
    }
