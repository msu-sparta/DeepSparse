...
dgraph *initializeNextGraph(coarsen *coars, MLGP_option opt)
{
    ...
    next_graph->vertices = (char**)malloc(sizeof(char*)*(nbnewnode+2));
...
coarsen *VCycle2way(dgraph *igraph, idxType* flag, const MLGP_option opt, MLGP_info* info)
{
    ...
    //opt.co_stop_size = 300;
    // printf("opt.co_stop_size = %d our vertex = %d opt.co_stop_level = %d our level = %d\n",opt.co_stop_size,current->graph->nVrtx, opt.co_stop_level,level); 
    
    while ( (current->graph->nVrtx > opt.co_stop_size) &&  (level < opt.co_stop_level)) {
       // printf("coarsening level loop entered\n");
        ...
        // for(i = 1 ; i <= nextgraph->nVrtx ; i++){
        //     for (j = nextgraph->inStart[i]; j <= nextgraph->inEnd[i]; ++j)
        //     {
        //         // code
        //         printf( "%s ---> %s (%lld)\n", nextgraph->vertices[nextgraph->in[j]-1],nextgraph->vertices[i-1], nextgraph->ecIn[j]);
        //     }
        // }
...
//    printf("Before initialBisection nVrtx = %d, nbpart = %d, lb0 = %f, ub0 = %f, lb1 = %f, ub1 = %f\n", 
 //       current->graph->nVrtx, opt.nbPart, opt.lb[0], opt.ub[0], opt.lb[1], opt.ub[1]);
    ...
    for ( ; level>=0 ;  level--) {
        ...
        if (opt.print >= PD_HIGH) {
            ...
            printf("Refinement:%2d: G(%6d, %6d) Cut=%lld\n", level, current->graph->nVrtx, current->graph->nEdge, (long long) edgecut);
        }
    ...
    if (opt.print >= PD_MEDIUM) {
        printf("Refinement:Final: Cut=%lld\n", (long long) info->final_ec);
    }
