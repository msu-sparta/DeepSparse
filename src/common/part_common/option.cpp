...
void printMLGPusage(char *exeName)
{
    ...
    //co_init
    printf("\t--inipart int \t(default = %d or %d if --conpar 1)\n", IP_DEF, IP_CONPAR);
    ...
    printf("\t\tDetermine if we anchor the sources and the targets after runing conpar\n");
    printf("\t\t= 0 : No\n");
    printf("\t\t= 1 : Yes\n");
    ...
    //live_comp
    printf("\t--live_comp int \t(default = 0)\n");
    printf("\t\tHow do we compute the liveset?\n");
    printf("\t\t= 0 : Lmin\n");
    printf("\t\t= 1 : L\n");
    printf("\t\t= 2 : Lmax\n");
    ...
    printf("\t\tCreate a file containing partition number for each node.\n");
...
int initMLGPoptions(MLGP_option *opt, int nbPart)
{
    ...
    printf("init opt stop size = %d const = %d nbpart = %d\n",opt->co_stop_size,CO_STOP_SIZE_MULT,opt->nbPart);
    ...
    opt->ub = (double*) umalloc((opt->nbPart)*sizeof(double), "opt->ub");
    opt->lb = (double*) umalloc((opt->nbPart)*sizeof(double), "opt->lb");
