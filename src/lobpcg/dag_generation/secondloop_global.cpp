...
/* gen_graph_v28.cpp using map (Each csb blcok is task): 
* 1. working on v27
* 2. Removing SPMMRED nodes for one buf version
*/
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    //char *filename = "ecology2.cus";
    //char *filename = "550k.cus";
    //char *filename = "graph100m1b.cus";
    //char *filename = "MatA100.cus";
    //char *filename = "MatA1083.cus";
    //char *filename = "../LOBPCG/mtx/Z5.N5.Nm6.Mj0.p15/Nm6.cus";
    //char *filename = "Nm6-littleendian.cus";

    char *filename = argv[5]; //"../Matrices/Nm7-original.cus";
    ...
    */
    ...
    /*std::ofstream graph_file("graph-v28.dot");
    ...
    for(i = 0 ; i < nodeCount ; i++)
    {
        ...
        if(name != "")
        {
            ...
            if (name.find("SPMM") != std::string::npos)
                graph_file << i + 1 << " [label=\"" << name << "\", color=deepskyblue, style = filled];" << endl;  
            else  if (name.find("XTY") != std::string::npos)
                graph_file << i + 1 << " [label=\"" << name << "\", color=salmon2, style = filled];" << endl;
            else  if (name.find("XY") != std::string::npos)
                graph_file << i + 1 << " [label=\"" << name << "\", color=brown1, style = filled];" << endl;
            else  if (name.find("ADD") != std::string::npos)
                graph_file << i + 1 << " [label=\"" << name << "\", color=chocolate1, style = filled];" << endl;
            else  if (name.find("SUB") != std::string::npos)
                graph_file << i + 1 << " [label=\"" << name << "\", color=darkorange, style = filled];" << endl;
            else  if (name.find("MULT") != std::string::npos)
                graph_file << i + 1 << " [label=\"" << name << "\", color=darkorchid2, style = filled];" << endl;
            else  if (name.find("UPDATE") != std::string::npos)
                graph_file << i + 1 << " [label=\"" << name << "\", color=forestgreen, style = filled];" << endl;
            else  if (name.find("GET") != std::string::npos)
                graph_file << i + 1 << " [label=\"" << name << "\", color=gold1, style = filled];" << endl;
            else  if (name.find("COL") != std::string::npos)
                graph_file << i + 1 << " [label=\"" << name << "\", color=magenta, style = filled];" << endl;
            else  if (name.find("SQRT") != std::string::npos)
                graph_file << i + 1 << " [label=\"" << name << "\", color=mediumseagreen, style = filled];" << endl;
            else
                graph_file << i + 1 << " [label=\"" << name << "\", color=wheat4, style = filled];" << endl;
            
            //without label
            if (name.find("SPMM") != std::string::npos)
                graph_file << i + 1 << " [label=\"""\", color=deepskyblue, style = filled];" << endl;  
            else  if (name.find("XTY") != std::string::npos)
                graph_file << i + 1 << " [label=\"""\", color=salmon2, style = filled];" << endl;
            else  if (name.find("XY") != std::string::npos)
                graph_file << i + 1 << " [label=\"""\", color=brown1, style = filled];" << endl;
            else  if (name.find("ADD") != std::string::npos)
                graph_file << i + 1 << " [label=\"""\", color=chocolate1, style = filled];" << endl;
            else  if (name.find("SUB") != std::string::npos)
                graph_file << i + 1 << " [label=\"""\", color=darkorange, style = filled];" << endl;
            else  if (name.find("MULT") != std::string::npos)
                graph_file << i + 1 << " [label=\"""\", color=darkorchid2, style = filled];" << endl;
            else  if (name.find("UPDATE") != std::string::npos)
                graph_file << i + 1 << " [label=\"""\", color=forestgreen, style = filled];" << endl;
            else  if (name.find("GET") != std::string::npos)
                graph_file << i + 1 << " [label=\"""\", color=gold1, style = filled];" << endl;
            else  if (name.find("COL") != std::string::npos)
                graph_file << i + 1 << " [label=\"""\", color=magenta, style = filled];" << endl;
            else  if (name.find("SQRT") != std::string::npos)
                graph_file << i + 1 << " [label=\"""\", color=mediumseagreen, style = filled];" << endl;
            else
                graph_file << i + 1 << " [label=\"""\", color=wheat4, style = filled];" << endl;
    ...
    std::ofstream globalgraph_file("global_graph_secondloop.txt");
    for(i = 0 ; i < globalNodeCount; i++)
    {
            ...
            char input1[], char input2[], char output[], int M, int N, int P, int block_width, int xy_id)
{
    ...
    ***********************************************/
    ...
    for(i = 0 ; i < nrowblks ; i++)
    {
        ...
        if(edge1Format == 1) //coming from a single variables
        {
            memset(&ary[0], 0, sizeof(ary));
            strcat(ary, edge1_var);
            strcat(ary, ",");
