...
double find_norm(double *V, int size)
{
    ...
    */
    ...
    cblas_dgemv(CblasRowMajor, CblasTrans, size, 1, 1.0, V,
            1, V, 1, 0, &rv, 1);
...
double find_dotp(double *V1, double *V2, int size)
{
    ...
    */
    ...
    cblas_dgemv(CblasRowMajor, CblasTrans, size, 1, 1.0, V1,
            1, V2, 1, 0, &rv, 1);
...
void xty_kernel(double *x, double *y, double *xty, int size, int width, int avail_eigs)
{
    /*
    int i, j;
    ...
    */
    ...
    cblas_dgemv(CblasRowMajor, CblasTrans, size, avail_eigs,
                1.0, x, width, y, 1, 0, xty, 1);
}

void xy_kernel(double *x, double *y, double *xy, int size, int width, int avail_eigs)
{
    /*
    int i, j;
    ...
    */
    ...
    cblas_dgemv(CblasRowMajor, CblasNoTrans, size, avail_eigs,
                1.0, x, width, y, 1, 0, xy, 1);
}
...
void Lanczos(char *filename, int eig_wanted)
{
    ...
    for (int iter = 0; iter != eig_wanted; ++iter)
    {
        ...
        xty_kernel(Q, z, QpZ_local, nrows, eig_wanted, iter+1);
        ...
        xy_kernel(Q, QpZ, QQpZ, nrows, eig_wanted, iter+1);
